...
import { Camera, CheckCircle, Info, Loader2, Sparkles, UtensilsCrossed } from 'lucide-react';
...
import { analyzeFridgeImage, generateRecipesFromItems } from '../../utils/fridgeAssistant';
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-gray-200 bg-white p-6 text-sm text-gray-500">
        Aucune détection pour le moment. Utilise la capture pour déclencher l'analyse.
      </div>
    );
  }
...
export function FridgeCapture() {
  ...
  const [recipes, setRecipes] = useState<GeneratedRecipeWithNutrition[]>([]);
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-gray-600">
          Photographiez l'intérieur du frigo, corrigez les aliments détectés et obtenez des idées healthy adaptées à votre
          objectif. Les calories & macros seront calculées à partir de la base interne (pas par l'IA).
        </p>
      </header>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm lg:col-span-2">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex-1 space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Sur mobile, le mode capture ouvre la caméra directement. L'analyse se fait côté serveur via OpenAI vision
                (ou fallback démo).
              </p>
            </div>
            <button
          ...
          <div className="mt-6 space-y-3">
            ...
            <DetectedList
              ...
              onIdChange={(index, id) =>
                setDetectedItems((prev) => prev.map((item, i) => (i === index ? { ...item, id: id || null } : item)))
              }
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-3 text-xs text-gray-500">
              Les macros seront calculées à partir de vos aliments enregistrés. Les sorties IA servent de suggestion et ne sont
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <div className="mb-2 flex items-center gap-2 font-semibold">
              ...
              <span>Compatibilité base aliments</span>
            </div>
            Vous pouvez corriger les noms/IDs pour coller à la base d'aliments (CIQUAL) avant de lancer la génération. Les
            calories seront toujours dérivées de la base interne.
          </div>
        </aside>
      </div>
